...
alakazam <- function(null_path, obs_path, out_file){

### SET VARS
annotation <- c("A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K")
rank <- c("A", "B", "C", "I", "D", "E", "F", "G", "H", "J", "K")
  
### IMPORT
# null_path = "~/Dropbox/PhD/Data/GWAS/formatted/GWAS_DDC_mouse_mapping_null_sampling_Hchr"
# obs_path = "~/Dropbox/PhD/Data/GWAS/formatted/GWAS_DDC_mouse_mapping.csv"
# out_file = "~/Dropbox/PhD/Data/Interspecific_SNV_mapping/GWAS_DDC_z_scores.csv"
dt_list <- list()
for (i in 1:22){
  dt_list[[i]] <- fread(paste0(null_path, i, ".csv"))
}
...
colnames(dt) <- c("H_annotation", "N_SNVs", "N_align", "N_conserved", "N_conserved_K", 
                  "A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "ind", "H_chromosome")
colnames(cv) <- c("H_CHR", "H_POS", "H_REF", "H_Kmer", "H_ANN", "M_CHR", "M_POS", "M_REF", "M_Kmer", "M_ANN")

# rank annotations
for (r in length(rank):1){
  dt$H_annotation_ranked[dt$H_annotation %like% rank[r]] <- rank[r]
  cv$H_annotation_ranked[cv$H_ANN %like% rank[r]] <- rank[r]
}

## sum null by annotation and int
ann_list <- list()
for (ann in 1:length(annotation)){
  sub <- subset(dt, dt$H_annotation_ranked == annotation[ann])
  ind_list <- list()
...
## sum SNVs by annotation
cv_list <- list()
for (i in 1:length(annotation)){
  sub <- subset(cv, cv$H_annotation_ranked == annotation[i])
  sub[sub==""] <- NA
  N_SNV_obs <- nrow(sub)
...
out2$H_annotation <- mapvalues(out2$H_annotation, from=c("A","B","C","D","E","F","G","H","I","J","K"),
                               to=c("Exon - CDS",
                                    "Exon - UTR",
                                    "Exon - other",
                                    "Promoter",
...
# colnames(out2)
out2 <- out2[,c("H_annotation", "N_SNV_obs", 
...
alakazam(null_path = "~/Dropbox/PhD/Data/GWAS/formatted/GWAS_DDC_mouse_mapping_null_sampling_Hchr",
         obs_path = "~/Dropbox/PhD/Data/GWAS/formatted/GWAS_DDC_mouse_mapping.csv",
         out_file = "~/Dropbox/PhD/Data/Interspecific_SNV_mapping/GWAS_DDC_z_scores.csv")

alakazam(null_path = "~/Dropbox/PhD/Data/GWAS/formatted/GWAS_nonDDC_mouse_mapping_null_sampling_Hchr",
         obs_path = "~/Dropbox/PhD/Data/GWAS/formatted/GWAS_nonDDC_mouse_mapping.csv",
         out_file = "~/Dropbox/PhD/Data/Interspecific_SNV_mapping/GWAS_nonDDC_z_scores.csv")
