...
### IMPORT DATA
data <- fread("~/Dropbox/PhD/Data/PC_constraint/Constraint/M_funZ_allMUSMUS_v2.csv")
...
# subset mouse funZ 
data <- data[,c("external_gene_name", "fun_Z")]
...
df$fun_Z_IMPC_percentile <- percentile(df$fun_Z)
...
# lm
summary(lm(df_mn_all$x~df_md_all$Category))
summary(lm(df_md_all$x~df_md_all$Category))
...
summary(lm(df_mn_top$x~df_md_all$Category))
summary(lm(df_md_top$x~df_md_all$Category))
...
Fig
save_plot("~/Dropbox/PhD/Data/PC_constraint/Figures_and_tables/PLoS/Figure_pleiotropy.jpg", Fig, ncol = 1, nrow = 1, base_height = 5, base_width = 10)
...
# 
# # subset mouse funZ 
# data <- data[,c("external_gene_name", "OE_ratio")]
# colnames(data) <- c("external_gene_name", "fun_Z")
# 
# # Merge data for plotting
# df <- merge(no.mp.tests, no.tlmp.tests)
# df <- merge(df, all.counts, all = T)
