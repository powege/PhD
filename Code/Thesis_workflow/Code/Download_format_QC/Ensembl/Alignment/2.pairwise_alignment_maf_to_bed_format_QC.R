...
# input.path <- "/well/lindgren/George/Data/Mapping_v1/raw/"
# output.path <- "/well/lindgren/George/Data/Mapping_v1/formatted/"
# dir <- "hsap_grch38.v.mmus_grcm38.lastz_net/"
# in.prefix <- "hsap_grch38.v.mmus_grcm38.lastz_net."
# out.prefix <- "hsap_grch38_v_mmus_grcm38_alignment_annotation_short_chr"
# chr.id <- 1
...
MAF_convert <- function(chr, files.chr, input.path, output.path, dir, in.prefix, out.prefix){
  ...
  for (j in 1:files.chr){
    ...
    colnames(dt_short) <- c("chromosome_human", "start_human", "end_human",
                            "chromosome_mouse", "start_mouse", "end_mouse")
    # write tables
    fwrite(dt_short, paste0(output.path, out.prefix, chr, ".bed.gz"), append = T, compress = "gzip")
