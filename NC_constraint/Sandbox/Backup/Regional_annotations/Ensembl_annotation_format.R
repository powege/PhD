...
### get unannotated POS ID

# functions
seq2 <- Vectorize(seq.default, vectorize.args = c("from", "to"))

sticks <- function(chr, dt){
  sub <- subset(dt, dt$chromosome == chr)
  all.POS <- unlist(seq2(from = sub$start, to = sub$end))
  all.CHR <- rep(chr, length(all.POS))
  out <- paste0(all.CHR, "_", all.POS)
  out <- unique(out)
  return(out)
}

unannotated <- fread("/well/lindgren/George/Data/NC_constraint/Constraint/MGP_constraint_by_window_700_100.csv")
# unannotated <- fread("~/Dropbox/PhD/Data/NC_constraint/Constraint/MGP_constraint_by_window_700_100.csv")
output.list <- list()
# for each chr:
for( chr in c(1:19)){
  
  # get all POS ID
  unannotated.sub <- unannotated[,c("CHR", "POS_from","POS_to")]
  colnames(unannotated.sub) <- c("chromosome", "start", "end")
  unannotated.sub <- sticks(chr, unannotated.sub)
  
  # identify POS not covered by each annotation. 
  files <- c("intron_POS_short.csv",
             "enhancer_POS_short.csv",
             "NC_exon_POS_short.csv",                   
             "open_chromatin_POS_short.csv",            
             "PC_exon_POS_short.csv",                   
             "promoter_flanking_POS_short.csv",         
             "promoter_POS_short.csv",                  
             "TF_binding_POS_short.csv",                
             "UTR_POS_short.csv")
  for (file in files){
    # annotated <- fread(paste0("~/Dropbox/PhD/Data/NC_constraint/Ensembl_annotation_POS/", file))
    annotated <- fread(paste0("/well/lindgren/George/Data/NC_constraint/Ensembl_annotation_POS/", file))
    annotated <- sticks(chr, annotated)
    unannotated.sub <- unannotated.sub[!unannotated.sub %in% annotated]
    print(file)
  }
  output.list[[chr]] <- unannotated.sub
  print (chr)
}
...
fwrite(output, "/well/lindgren/George/Data/NC_constraint/Ensembl_annotation_POS/unannotated_POS_ID_long.csv")
