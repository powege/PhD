...
### IMPORT
dt <- read.csv("~/Dropbox/PhD/Data/NC_constraint/Figures_and_tables/Raw/GWAS_SNVs_by_percentile.csv")

### FORMAT

# calculate fold change vs the 100th percentile
dt$fold_change <- NA
dt$fold_change[dt$species == "Human"] <- dt$n_gwas_adj[dt$species == "Human"] / dt$n_gwas_adj[dt$species == "Human" & dt$percentile == 100]
dt$fold_change[dt$species == "Mouse"] <- dt$n_gwas_adj[dt$species == "Mouse"] / dt$n_gwas_adj[dt$species == "Mouse" & dt$percentile == 100]

# group percentiles with custom function
dt_plot <- rbind(
  gobbler(dt[dt$species == "Human",]),
  gobbler(dt[dt$species == "Mouse",])
)

### PLOT 1

p_1 <- ggplot(dt_plot, aes(x=rank, y=mean_fold_change, color=species)) + 
  # geom_point() +
  geom_line(size=1.5) +
...
p_1
ggsave("~/Dropbox/PhD/Data/NC_constraint/Figures_and_tables/Figure_GWAS_by_percentile.jpg", plot = p_1, height = 5, width = 6)
