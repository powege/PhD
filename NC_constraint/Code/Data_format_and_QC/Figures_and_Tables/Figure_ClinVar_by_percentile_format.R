...
# HM.clinvar.file <- args[2]
# out.file <- args[3]
H.clinvar.file <- "~/Dropbox/PhD/Data/ClinVar/formatted/ClinVar_pathogenic_formatted.vcf"
HM.clinvar.file <- "~/Dropbox/PhD/Data/ClinVar/formatted/ClinVar_human_to_mouse_alignment.txt"
...
out.file <- "~/Dropbox/PhD/Data/NC_constraint/Figures_and_tables/Raw/ClinVar_SNVs_by_percentile.csv"
...
HM_CV_H <- HM_CV[,c("CHR","POS","REF","ALT")]
...
dt <- HM_CV[H_CV, on = c("H_CHR", "H_POS", "H_REF", "H_ALT", "H_Constraint_percentile")]
dt <- dt[!duplicated(dt),]
...
dt$H_CAT[dt$H_CAT != "Exon - CDS"] <- "Non-coding"
dt$H_CAT[dt$H_CAT == "Exon - CDS"] <- "Protein-coding"
dt$M_CAT[dt$M_CAT != "Exon - CDS"] <- "Non-coding"
dt$M_CAT[dt$M_CAT == "Exon - CDS"] <- "Protein-coding"
...
M_n_PC <- rep(NA, 100)
M_n_NC <- rep(NA, 100)
H_PC <- subset(dt, dt$H_CAT == "Protein-coding")
H_NC <- subset(dt, dt$H_CAT == "Non-coding")
M_PC <- subset(dt, dt$M_CAT == "Protein-coding")
M_NC <- subset(dt, dt$M_CAT == "Non-coding")

for(i in percentile){
  tmp <- subset(H_PC, H_PC$H_Constraint_percentile == i)
  H_n_PC[i] <- nrow(tmp)
  tmp <- subset(H_NC, H_NC$H_Constraint_percentile == i)
  H_n_NC[i] <- nrow(tmp)
  tmp <- subset(M_PC, M_PC$M_Constraint_percentile == i)
  M_n_PC[i] <- nrow(tmp)
  tmp <- subset(M_NC, M_NC$M_Constraint_percentile == i)
  M_n_NC[i] <- nrow(tmp)
}

out <- data.frame(percentile = c(percentile, percentile, percentile, percentile),
                  n_ClinVar = c(H_n_PC, H_n_NC, M_n_PC, M_n_NC),
                  fraction = c(rep(1, 200), M_frac$fraction, M_frac$fraction),
                  species = c(rep("Human", 200), rep("Mouse", 200)),
                  annotation = c(rep("Protein-coding", 100),
                                 rep("Non-coding", 100),
                                 rep("Protein-coding", 100),
                                 rep("Non-coding", 100)))
out$n_ClinVar_adj <- out$n_ClinVar / out$fraction
...
#                      N_ClinVar = c(H_all, H_align, M_align),
#                      Annotation = c(rep("Human ClinVar all", 100), rep("Human ClinVar aligned", 100), rep("Mouse ClinVar aligned", 100)))
