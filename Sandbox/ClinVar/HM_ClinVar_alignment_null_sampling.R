...
# CV_file <- "~/Dropbox/PhD/Data/Interspecific_SNV_mapping/GWAS_mouse_mapping_Hchr1.csv"
...
sample_sample <- function(i, H_ref_cat, CV_cats){
  ...
  return(c(category, total_H, total_M_align, total_M_conserv, total_M_conserv_K,
           ...
           total_M_align_F, total_M_align_G, total_M_align_H, total_M_align_I, total_M_align_J))
}
...
for (i in 1:nrow(CV_cats)){
  ...
  AA <- replicate(10000, sample_sample(i = i, H_ref_cat = H_ref_cat, CV_cats = CV_cats), simplify = FALSE)
  # index output
