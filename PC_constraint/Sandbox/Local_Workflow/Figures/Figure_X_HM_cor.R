...
o2o <- subset(df, df$orthology_type == "ortholog_one2one")
o2m <- subset(df, df$orthology_type == "ortholog_one2many")
m2m <- subset(df, df$orthology_type == "ortholog_many2many")

cor.test(df$M_fun_Z_0.0001, df$H_fun_Z_0.001)$p.value
cor.test(o2o$M_fun_Z_0.0001, o2o$H_fun_Z_0.001)
plot(o2m$M_fun_Z_0.0001, o2m$H_fun_Z_0.001)
cor.test(o2m$M_fun_Z_0.0001, o2m$H_fun_Z_0.001)
plot(m2m$M_fun_Z_0.0001, m2m$H_fun_Z_0.001)
cor.test(m2m$M_fun_Z_0.0001, m2m$H_fun_Z_0.001)
...
ggsave("~/Dropbox/BitBucket_repos/phd/PC_constraint/Paper/Results/Figures/Figure_2.jpg", plot = Fig2, width = 6, height = 6)
...
H_HMC <- HMC[!duplicated(HMC$H_external_gene_name),]
table(H_HMC$orthology_type)
M_HMC <- HMC[!duplicated(HMC$M_external_gene_name),]
table(M_HMC$orthology_type)
...
H_HMU <- HMU[!duplicated(HMU$H_external_gene_name),]
table(H_HMU$orthology_type)
M_HMU <- HMU[!duplicated(HMU$M_external_gene_name),]
table(M_HMU$orthology_type)
