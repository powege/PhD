...
plotB
ggsave("~/Dropbox/BitBucket_repos/phd/PC_constraint/Paper/Results/Figures/Figure_3B.jpg", plot = plotB, width = 6.5, height = 6)
...
colnames(ROC_df2) <- c("H_external_gene_name", "obs", 
                       paste0("Multivariate GLM AUC=0.82\n", 
                              "    pLI  p=", p.pLI, "\n",
...
plotC <- ggplot(ROC_df2, aes(d = obs, m = value, color = variable)) + 
  ...
  theme(legend.key.size = unit(1, "cm"),
        ...
        plot.margin=unit(c(1,1,1,1),"cm"))
plotC
ggsave("~/Dropbox/BitBucket_repos/phd/PC_constraint/Paper/Results/Figures/Figure_3C.jpg", plot = plotC, width = 6.5, height = 6)
