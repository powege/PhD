...
df$HM_funZ_class <- "NA"
df$HM_funZ_class[df$H_fun_Z_percentile_orth >= 0.9 & df$M_fun_Z_percentile_orth >= 0.9] <- "Most constrained"
df$HM_funZ_class[df$H_fun_Z_percentile_orth <= 0.1 & df$M_fun_Z_percentile_orth <= 0.1] <- "Least constrained"
...
cor.test(df.plot$Conservation, df.plot$M_fun_Z_0.0001, method = "spearman")
spearman <- spear_corr_eqn(df.plot$Conservation, df.plot$M_fun_Z_0.0001, digits = 3)

M_Fig <- ggplot(df.plot, aes(x = M_fun_Z_0.0001, y = Conservation)) +
...
cor.test(df.plot$Conservation, df.plot$H_fun_Z_0.001, method = "spearman")
spearman <- spear_corr_eqn(df.plot$Conservation, df.plot$H_fun_Z_0.001, digits = 3)

H_Fig <- ggplot(df.plot, aes(x = H_fun_Z_0.001, y = Conservation)) +
  geom_point(aes(col=HM_funZ_class)) +
  geom_smooth(method='lm', formula=y~x, se = T, colour = "black", size = 0.6, fullrange = T) +
  # annotate("text", x = -4, y = 8, label = lm_equation, colour="black", size = 5, parse=TRUE) +
  annotate("text", x = -7, y = 8, label = spearman, colour="black", size = 5, parse=TRUE) +
...
out <- plot_grid(M_Fig, H_Fig, labels = c("Mouse", "Human"),
                                     ncol = 2, nrow = 1)
out
save_plot("~/Dropbox/BitBucket_repos/phd/PC_constraint/Paper/Results/Supplementary/Figure_funZ_conserv_cor.jpg", 
