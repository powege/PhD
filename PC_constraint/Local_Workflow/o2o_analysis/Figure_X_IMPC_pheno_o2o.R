...
temp$color <- "white"
temp$color[temp$top_level_mp_term_name == "all IMPC knockouts (n=4889)"] <- "blue"
...
fig1 <- ggplot(temp, aes(x=top_level_mp_term_name, y=fun_Z_IMPC_percentile, fill = color)) +
  ...
  stat_compare_means(label = "p.signif", method = "wilcox",
                     ref.group = "all IMPC knockouts (n=4889)",
                     size = 6,
                     hide.ns = T) +
...
fig1
# ggsave("~/Dropbox/BitBucket_repos/phd/PC_constraint/Paper/Results/Figures/Figure_5A.jpg", plot = fig1, height = 6, width = 8)
