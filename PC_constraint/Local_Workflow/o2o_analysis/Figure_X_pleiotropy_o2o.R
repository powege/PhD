### Relationship between constraint and number of pheotypes (total, and unique top-level)
...
# get o2o orthologues
orths <- orths[orths$orthology_type == "ortholog_one2one"]
orths <- orths$M_external_gene_name
df <- df[df$external_gene_name %in% orths,]
...
plot_all <- ggplot(df_md_all, aes(x = Category, y = x)) +
  ...
  ylab('Median phenotypes per knockout') +
  # xlim(-10, 10) +
  # ylim(-10, 10) +
  theme_bw() +
...
plot_top <- ggplot(df_md_top, aes(x = Category, y = x)) +
  ...
  xlab("FunZ percentile bin") +
  ylab('Median unique top-level phenotypes per knockout') +
  # xlim(-10, 10) +
  # ylim(-10, 10) +
  theme_bw() +
  theme(legend.position="none",
...
Fig
save_plot("~/Dropbox/BitBucket_repos/phd/PC_constraint/Paper/Results/Figures/Figure_pleiotropy_o2o.jpg", Fig, ncol = 1, nrow = 1, base_height = 5, base_width = 10)
