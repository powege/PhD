...
count_7mer <- function(anc.SEQ, k7_all){
  ...
  b6 <- anc.SEQ[5:(length(anc.SEQ)-1)]
  b7 <- anc.SEQ[5:(length(anc.SEQ))]
...
write.csv(k7_out, "~/Dropbox/BitBucket_repos/phd/PC_constraint/Paper/Data/Mu_rates/M_7mer_mu_rate.table", row.names = F)

# HUMAN
chr <- c(1:22, "X")
in_file_path   <- "/Volumes/HarEx/PC_constraint/Paper/Data/Ancestral_mutant/H_ancestral_mutant_SEQ_chr"
k7_out <- k7_mu_rr(chr, in_file_path)
write.csv(k7_out, "~/Dropbox/BitBucket_repos/phd/PC_constraint/Paper/Data/Mu_rates/H_7mer_mu_rate.table", row.names = F)
