...
plotter <- function(m_df_md, m_df_mn, lm_equation){
  
  m_plot <- ggplot() +
    geom_bar(data=m_df_md, aes(y=x,x=Category,color="red"), stat="identity", width = 0.75) + 
    geom_point(data=m_df_mn, aes(y=x, x=Category)) +
    geom_smooth(data=m_df_mn, aes(y=x, x=Category), method='lm') +
...
m_plot <- plotter(m_df_md, m_df_mn, m_lm_equation)
m_plot
h_plot <- plotter(h_df_md, h_df_mn, h_lm_equation)
h_plot

Fig <- plot_grid(h_plot, m_plot, ncol = 2, nrow = 1, labels = c("Human", "Mouse"), 
