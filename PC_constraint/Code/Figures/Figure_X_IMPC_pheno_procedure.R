...
no_pheno_genes <- data.frame(external_gene_name = no_pheno_genes,
                             procedure_name = rep("no annotated phenotype", length(no_pheno_genes)))
...
impc <- impc[, c("marker_symbol", "procedure_name", "procedure_stable_id", "p_value", "effect_size", "percentage_change")]
colnames(impc) <- c("external_gene_name", "procedure_name", "procedure_stable_id", "p_value", "effect_size", "percentage_change")
# remove genes with no top level phenotype term
impc <- impc[!is.na(impc$procedure_name),]
impc <- impc[impc$procedure_name != "",]
...
temp <- temp[temp$procedure_name %in% names(which(table(temp$procedure_name) > 49)), ]
summary(factor(temp$procedure_name))

# add reference with all genes
ref <- temp[!duplicated(temp$external_gene_name),]
ref$procedure_name <- "all IMPC knockouts"
...
temp$procedure_stable_id[temp$procedure_name == "no annotated phenotype"] <- ""
temp$procedure_stable_id[temp$procedure_name == "all IMPC knockouts"] <- ""
temp <- na.omit(temp)
n.temp <- as.data.frame(table(temp[,"procedure_name"]))
colnames(n.temp) <- c("procedure_name", "N_genes")
temp <- merge(temp, n.temp)
temp$procedure_name <- paste0(temp$procedure_name, " (n=", temp$N_genes, ")")
table(temp$procedure_name)
# temp$procedure_name[temp$procedure_name == "no annotated phenotype () (n=1339)"] <- "no annotated phenotype (n=1339)"
# temp$procedure_name[temp$procedure_name == "all IMPC knockouts () (n=5479)"] <- "all IMPC knockouts (n=5479)"
temp$procedure_name <- factor(temp$procedure_name)
summary(temp$procedure_name)


# set order of top-level terms
# order <- reorder(temp$top_level_mp_term_name, temp$fun_Z_IMPC_percentile, FUN = median, order = T)
order <- aggregate(temp$fun_Z_IMPC_percentile~temp$procedure_name, FUN=median)
colnames(order) <- c("procedure_name", "fun_Z_IMPC_percentile")
order <- order[order(order$fun_Z_IMPC_percentile),]
order <- order$procedure_name[c(2, 1, 3:nrow(order))]
temp$procedure_name <- factor(temp$procedure_name, levels = order)

# add colour category for baseline
temp$color <- "white"
temp$color[temp$procedure_name == "all IMPC knockouts (n=5207)"] <- "blue"

### PLOT IMPC percentile
fig1 <- ggplot(temp, aes(x=procedure_name, y=fun_Z_IMPC_percentile, fill = color)) +
  geom_boxplot() +
  scale_fill_manual(values=c("skyblue", "white")) +
  stat_compare_means(label = "p.signif", method = "wilcox",
                     ref.group = "all IMPC knockouts (n=5207)",
                     size = 6,
                     hide.ns = T) +
  xlab("IMPC procedure") +
...
fig1
ggsave("~/Dropbox/BitBucket_repos/phd/PC_constraint/Paper/Results/Figures/Figure_X_IMPC_procedure.jpg", plot = fig1, height = 8, width = 10)
